...
import { routerReducer, createRouterMiddleware, RouterHistoryContainer } from 'redux-router-kit';
...
const enhancers = routes =>
  composeWithDevTools(applyMiddleware(createRouterMiddleware({ routes }), reduxLogger));

// Create the Redux store
const store = createStore(reducerStack(appReducers.reducer), enhancers(appRoutes));
...
if (process.env.NODE_ENV !== 'production') {
  ...
  const devReducerStack = (appReducer) => {
    const reducer = reducerStack(appReducer);
    return (...args) => deepFreeze(reducer(...args));
  };
  store.replaceReducer(devReducerStack(appReducers.reducer));
  ...
  if (module.hot) {
    ...
    module.hot.accept('./reducers', () => {
      ...
      store.replaceReducer(devReducerStack(newAppReducers.reducer));
    });
